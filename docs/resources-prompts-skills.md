...
Run `make verify-public-surface` from a source checkout to verify the public MCP surface. The target builds the server, checks generated artifacts, runs the focused MCP/release-pack tests, and exercises the Codex and Claude native smoke profiles.
...
Public source imports should include the generated skill and prompt documentation that comes from those canonical workflow definitions, but should continue to exclude local schema pulls, fetched constants, and any other restricted STRATZ-derived artifacts until clearance permits redistribution.
